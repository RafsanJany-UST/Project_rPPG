...
from Main.rPPG_Algorithm_Cell import rppg_chrom
...
# professor's "small drift" threshold in frames
...
def compute_edge_lags_for_sequence(seq_id: str):
    ...
    print(f"\n=== UBFC edge lag diagnostic for {seq_id} (CHROM) ===")
    ...
    # 7) We compute remaining frames at the tail and make our decision
    ...
    if delta_lag_frames <= SMALL_DRIFT_THR_FRAMES:
        decision = "TRIM"          # small drift → we can discard remaining frames
    elif abs(delta_lag_frames - remaining_frames) <= SMALL_DRIFT_THR_FRAMES:
        decision = "FPS_ADJUST"    # drift ~ remaining frames → we adjust FPS
    else:
        decision = "UNCERTAIN"     # we need further inspection
    ...
    return {
        ...
        "fps_start": fps_start,
        "fps_end": fps_end,
        "extra_tail_time": extra_tail_time,
